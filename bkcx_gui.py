...
def showres(score_df, args):
	...
	if args.count('') < 5:
		...
		input_opt['分数线'] = args[3].strip()
		input_opt['位次'] = args[4].strip()
		print(input_opt)
...
def convertinput(inputs):
	'''转换为List'''
	return str(inputs).strip().split()

def main(score_df):
	with gr.Blocks() as demo:
		gr.Markdown("# 浙江省2020-2023高考一段线信息查询系统")
		gr.Markdown(helper)
		with gr.Row():
